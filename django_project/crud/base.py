...
class CrudBaseMixin(CrispyLayoutFormMixin):
    ...
    def url_name_set(self, suffix):
        ...
        return '%s_%s' % (getattr(obj.model, obj.parent_field
                                  ).field.model._meta.model_name, suffix)

    def resolve_url_set(self, suffix, args=None):
        obj = self.crud if hasattr(self, 'crud') else self
        namespace = getattr(
            obj.model, obj.parent_field).field.model._meta.app_config.name
...
class CrudListView(GroupRequiredMixin, LoginRequiredMixin, ListView):
    ...
    group_required = ['Supervisor', 'Aluno', 'Recepcionista']
...
class CrudUpdateView(GroupRequiredMixin,
    ...
    raise_exception = True
    login_url = LOGIN_REDIRECT_URL
    group_required = ['Supervisor', 'Recepcionista']
